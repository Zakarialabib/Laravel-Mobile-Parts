<div>
    <nav class="flex items-center justify-between bg-gray-50 border-b px-4 lg:px-12 py-5 shadow-md">
        <div class="lg:mr-8 2xl:mr-20">
        <a class="lg:text-3xl sm:text-xl font-bold font-heading text-black"
            href="{{ route('front.index') }}">
            <img class="w-auto h-10" src="{{ asset('images/' . Helpers::settings('site_logo')) }}" loading="lazy"
                alt="{{ Helpers::settings('site_title') }}" />
        </a>
        </div>
        <div class="hidden md:contents items-center text-gray-900 space-x-2">
    ...
    <div class="hidden navbar-menu fixed top-0 left-0 bottom-0 w-5/6 max-w-sm z-50">
        ...
        <nav class="relative flex flex-col py-6 px-6 w-full h-full bg-white border-r overflow-y-auto">
            ...
            <ul class="lg:text-3xl sm:text-xl font-bold font-heading">
                <li class="mb-8 hover:underline"><a href="{{ route('front.categories') }}">{{ __('Categories') }}</a>
                </li>
            ...
            <div class="flex justify-between">
                ...
                @else
                    ...
                    <div class="w-full lg:text-3xl sm:text-xl font-bold font-heading">
                        ...
                        {{ __('or') }}
                        <div class="py-3">
                            <a class="hover:text-green-500" href="{{ route('register') }}"> {{ __('Register') }}</a>
                        </div>
                    </div>
                @endif
            </div>
